...
- **Manual anchor** — pick the peak you believe is a known ion (default `[M+H]+` /
  ...
  isotope (up to **Max isotope M+n**) and common in-source neutral loss is
  projected onto the spectrum; peaks within tolerance are labelled. This is the
  most reliable mode — *you* decide the molecular ion, avoiding false hits on
  noisy raw data.
...
that fraction of the base peak before matching, and **Annotate only top N peaks**
caps annotation to the N most intense — both keep noise from being labelled.
Labels are written vertically and multiple hits on one peak are joined with "; ".
**In-source fragments** and **Show expected-but-absent** (faint ghost ticks at
projected but unobserved positions) are toggle-able. Annotations are part of the
