...
#' Project the adduct and in-source-fragment ions for a neutral mass M from
#' commonMZ::MZ_CAMERA, classifying each row by mass (is_fragment_rule). Isotopes
#' are not projected; they are DETECTED after matching (add_detected_isotopes).
#' Returns tibble(label, type = "adduct"|"fragment", mz, charge).
#' @param M neutral monoisotopic mass.
#' @param mode "pos" / "neg".
#' @param fragments include in-source-fragment rows (e.g. `[M+H-H2O]+`).
...
#' @param iso_tol per-step isotope m/z tolerance (Da).
#' @param iso_decreasing require each isotope to be no more intense than the prior.
#' @noRd
annotate_anchor <- function(spec_df, anchor_mz, adduct = NULL, mode = c("pos", "neg"),
  ...
  # Match against the DEISOTOPED, centroided spectrum so an isotope peak can never
  # be labelled as an adduct/fragment (isotope wins); its satellites are labelled
  # separately from the full spectrum.
  cp <- centroid_peaks(spec_df, rel_floor = 0)
  zmax <- if (is.finite(max_charge)) max(1L, as.integer(max_charge)) else 2L
  pool <- deisotope(cp, iso_tol, max_charge = zmax)
  tab <- match_spectrum(pool, expected, tol, unit)
  if (isotopes >= 1)
    tab <- add_detected_isotopes(tab, cp, isotopes, iso_tol, iso_decreasing)
...
#' isotope-spaced pair (delta ~= k * 13C, k = 1..3, within k * `iso_tol`) is
#' skipped — covering the case where the satellite is the taller or a noise peak.
#' When `iso_decreasing`, a near-isotope delta is only skipped if the heavier peak
#' is no more intense than the lighter one (the isotope direction); this lets a
#' real -H2 loss — heavier peak more intense — through as a fragment.
#' @importFrom tibble tibble
#' @noRd
difference_network <- function(spec_df, tol = 10, unit = "ppm", top_n = 30L,
                               rel_floor = 0.01, ignore_isotopes = TRUE,
                               iso_tol = ISO_TOL_DA, iso_decreasing = TRUE) {
  empty <- tibble(mz_lo = numeric(), mz_hi = numeric(), delta = numeric(),
                  origin = character(), int_lo = numeric(), int_hi = numeric())
  cp <- centroid_peaks(spec_df, rel_floor = rel_floor)
  if (isTRUE(ignore_isotopes)) cp <- deisotope(cp, iso_tol)
  ...
  for (i in seq_len(nrow(cp) - 1L)) for (j in (i + 1L):nrow(cp)) {
    ...
    if (ignore_isotopes) {
      # Skip a near-isotope-spaced delta (k = 1..3, window k * iso_tol since the
      # spacing error compounds). With iso_decreasing, only when the heavier peak
      # is no more intense than the lighter (isotope direction) — so a -H2 loss
      # (heavier peak more intense) is kept as a fragment.
      ik <- round(delta / ISOTOPE_SPACING)
      if (ik >= 1 && ik <= 3 && abs(delta - ik * ISOTOPE_SPACING) <= ik * iso_tol &&
          (!iso_decreasing || int_hi <= int_lo)) next
    }
...
#' k * `iso_tol` AND is at least as intense (the monoisotope is the tallest for
#' small molecules). Checking the lower steps cascades to strip a whole envelope to
#' its monoisotope. The window scales with k because the spacing error compounds.
#' Position+intensity based and governed by `iso_tol` — deliberately NOT
#' MetaboCoreUtils::isotopologues, whose theoretical-spacing grouping is too strict
#' for real MS2 (centroids tens of ppm off ideal). Returns a logical over `cp`.
#' @noRd
isotope_satellites <- function(cp, iso_tol = ISO_TOL_DA, max_charge = 2L,
                               max_k = 2L) {
  n <- nrow(cp)
  if (n < 2) return(rep(FALSE, n))
  o <- order(cp$mz); mz <- cp$mz[o]; it <- cp$intensity[o]
  sat <- logical(n)
  span <- max_k * ISOTOPE_SPACING + max_k * iso_tol   # widest look-behind window
  for (a in seq_len(n)) {
    lo <- findInterval(mz[a] - span, mz) + 1L          # candidate parents below
    if (lo > a - 1L) next
    for (b in lo:(a - 1L)) {
      if (it[b] < it[a]) next                          # parent must be >= intensity
      d <- mz[a] - mz[b]
      for (z in seq_len(max_charge)) for (k in seq_len(max_k))
        if (abs(d - k * ISOTOPE_SPACING / z) <= k * iso_tol) { sat[o[a]] <- TRUE; break }
      if (sat[o[a]]) break
    }
  }
  sat
...
#' @noRd
deisotope <- function(cp, iso_tol = ISO_TOL_DA, max_charge = 2L) {
  if (nrow(cp) < 2) return(cp)
  cp[!isotope_satellites(cp, iso_tol, max_charge), , drop = FALSE]
}
...
#' within k * `iso_tol`. Position-anchored (not isotopologue grouping) because real
#' MS2 isotope centroids are often tens of ppm off the theoretical spacing. When
#' `decreasing` (the usual biological case) each successive isotope must be no more
#' intense than the previous member, which keeps a -H2 loss from being read as an
#' isotope; otherwise a member may be up to 1.5x the monoisotope.
#' @importFrom tibble tibble
#' @noRd
add_detected_isotopes <- function(tab, cp, n = 2L, iso_tol = ISO_TOL_DA,
                                  decreasing = TRUE) {
  ...
  for (i in seq_len(nrow(hits))) {
    ...
    mono_mz <- hits$mz_obs[i]; prev_int <- hits$intensity[i]
    for (k in seq_len(n)) {
      em <- mono_mz + k * ISOTOPE_SPACING / z
      cand <- which(abs(cp$mz - em) <= k * iso_tol)
      if (!length(cand)) break                        # envelope ends; stop this ion
      j <- cand[which.min(abs(cp$mz[cand] - em))]
      ok <- if (decreasing) cp$intensity[j] <= prev_int
            else cp$intensity[j] <= 1.5 * hits$intensity[i]
      if (!ok) break
      prev_int <- cp$intensity[j]
