...
- **Auto-suggest (findMAIN)** — uses
  ...
  another row to switch. (Auto mode has no manual anchor box.) **Climb water-loss
  ladder** (on by default) steps up the +H₂O series from the suggested ion to the
  heaviest peak — findMAIN only knows a single water loss, so for spectra dominated
  by sequential in-source water losses it otherwise anchors a lower rung and
  under-calls the neutral mass.
