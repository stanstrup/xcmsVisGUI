...
**Profile-mode spectra** filter, default *Auto*, which peak-picks for the MS map
but not here). It is drawn as a continuous **line** — which is what profile data
actually is, rather than a stick per detector sample — and the title is marked
*profile*. Clicking a peak on a raw profile trace snaps to the peak apex, so the
*m/z* you send to the EIC list or use as an annotation anchor is the real one and
not a flank sample. Set the filter to *Always centroid* to peak-pick this view too
and get a conventional stick plot. See
