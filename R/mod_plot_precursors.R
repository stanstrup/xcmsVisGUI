  ...
  moduleServer(id, function(input, output, session) {
    ...
    plot_gg <- reactive({
      ...
      withProgress(message = "Reading precursors…", value = 0.5, {
        df <- compute_precursors(f$path)
      })
      validate(need(nrow(df) > 0, "No precursor ions found in this file."))
      df$.tip <- sprintf("precursor m/z: %.4f\nrt: %.0f s", df$precursorMZ, df$rt)
      ggplot2::ggplot(df, ggplot2::aes(x = rt, y = precursorMZ, text = .tip)) +
        ggplot2::geom_point(color = brewer_qual(1, rv$settings$qual_palette),
                            alpha = 0.5, size = 1.2) +
        ggplot2::labs(x = "retention time (s)", y = "precursor m/z") +
        ggplot2::theme_bw()
    })
