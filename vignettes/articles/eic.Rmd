...
- **Scale intensity** — how the y-axis is scaled:
    ...
    - *Normalise each trace (÷ own max)* — every EIC peaks at 1, so you compare
      peak **shapes** / co-elution regardless of abundance.
    - *Normalise per target (÷ target max)* — a compound's traces across files share
      one 0–1 scale, so their **relative abundances** are comparable.
    - *Log10 (y-axis)* — `log10(intensity + 1)`, compressing the dynamic range so
      weak traces are visible (the +1 keeps the many baseline zeros).
