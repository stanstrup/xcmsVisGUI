...
~17 000 points, and a whole file ~29 million. So **Profile-mode spectra** in the
Filters panel defaults to *Centroid if profile (auto)* — profile scans are
peak-picked before the **Spectrum** and **MS map** views draw them, which makes
the spectrum a clean stick plot and the map roughly 15× smaller and faster. Only
the levels that are actually profile are picked, so a mixed file keeps its
centroided MS2 spectra untouched.

Choose *Never — show raw profile* to see the unprocessed trace instead: the
spectrum view then draws it as a continuous line (not sticks, which is what
profile data really is) and its title is marked *profile*. Clicking a peak still
gives you the true peak *m/z* — the click snaps to the apex rather than to
whichever flank sample you happened to hit. Be aware the MS map will then plot
every raw sample, which is slow. *Always centroid* forces peak picking even on
files that claim to be centroided already.
