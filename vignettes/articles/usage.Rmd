...
- **Manual anchor** — pick the peak you believe is a known ion (default `[M+H]+` /
  ...
  isotope (up to **Max isotope M+n**) and in-source fragment — all from the single
  [commonMZ](https://github.com/stanstrup/commonMZ) dictionary — is projected onto
  the spectrum; peaks within tolerance are labelled. This is the most reliable
  mode — *you* decide the molecular ion, avoiding false hits on noisy raw data.
- **Auto-suggest (findMAIN)** — uses
  ...
  and isotope support). Press **Suggest molecular ion**; the top-scoring
  hypothesis is annotated immediately — click another row to switch. (Auto mode
  has no manual anchor box; it is driven entirely by the ranked table.)
...
The **± tol** is the match window (ppm or Da); **Min intensity** drops peaks below
that fraction of the base peak before matching; **Max charge** limits the charge
states projected; and **Annotate only top N peaks** keeps the N most intense
*annotated* peaks. Labels are written vertically and multiple hits on one peak are
joined with "; ". **In-source fragments** and **Show expected-but-absent** (faint
ghost ticks at projected but unobserved positions) are toggle-able. Annotations are
part of the
plot, so they carry through to exports.
