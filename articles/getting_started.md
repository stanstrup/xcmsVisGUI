...
- [TIC /
  BPC](https://stanstrup.github.io/xcmsVisGUI/articles/tic_bpc.md) —
  total- and base-peak chromatograms
- [EIC](https://stanstrup.github.io/xcmsVisGUI/articles/eic.md) —
...
- [MS map](https://stanstrup.github.io/xcmsVisGUI/articles/ms_map.md) —
  2D and 3D *m/z* × time maps
- [Precursors](https://stanstrup.github.io/xcmsVisGUI/articles/precursors.md)
  — DDA precursor-ion map

The screenshots come from example LC-MS runs — a positive-mode QTOF
urine series and the `faahKO` / `msdata` demo datasets that ship with
Bioconductor.

## Launch

Install it first (see the
[README](https://github.com/stanstrup/xcmsVisGUI#install)), then the app
...
A browser tab opens with five plot views across the top (**TIC/BPC**,
**EIC**, **Spectrum**, **MS map**, **Precursors**), a **Settings** page,
and a left sidebar with **Files** and **Filters**. The sidebar and
...
- **Paste a folder or file path** and click **Add** — loads every MS
  file in a folder (`.mzML` / `.mzXML` / `.CDF`), or a single file if
  you paste a file path.
...
- **Click a chromatogram trace** (TIC/BPC or EIC), **a map pixel** (MS
  map) or **a precursor** (Precursors) and the **Spectrum** tab is
  loaded with that file and retention time — switch to the Spectrum tab
  to view it.
- **Click a peak in a spectrum** to add its *m/z* to the **EIC** target
  list (the default), then open the EIC tab to extract it across files.
...
the on-screen plotly — using the size/format defaults from Settings. You
can also save the raw ggplot object (`.rds`) to tweak later in R.
