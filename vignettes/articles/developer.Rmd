...
description: >
  Status, running from a clone, tests, the renv lockfile, and the project layout.
...
one-shot installer — it installs everything declared in `DESCRIPTION` (Imports +
Suggests and the `commonMZ` GitHub remote), picks the Bioconductor release that
matches your R, and re-snapshots `renv.lock`:

```{r, eval = FALSE, echo = TRUE}
# Rscript renv-setup.R
```

To reproduce the exact pinned versions from the lockfile on the **same** R
version, use `renv::restore()`. (renv libraries are R-version-specific, so an R
upgrade needs a fresh install via `renv-setup.R` rather than `restore()`.)
