...
~17 000 points, and a whole file ~29 million. **Profile-mode spectra** in the
Filters panel controls what happens about that, and defaults to *Auto*, which
treats the two views differently because they want opposite things:

- The **Spectrum** view shows the scan **raw** — you opened it to look at the
  data, and one scan is cheap to draw. It is drawn as a continuous **line**, which
  is what profile data actually is (not sticks), and the title is marked *profile*.
  Clicking a peak still gives you the true peak *m/z*: the click snaps to the apex
  rather than to whichever flank sample you happened to hit.
- The **MS map** **peak-picks** first. It has to: mapping every raw sample means
  tens of millions of points per file. Only the levels that really are profile get
  picked, so a mixed file keeps its centroided MS2 spectra untouched.

Override it with *Always centroid* (peak-pick everywhere, including the Spectrum
view — and even on files that claim to be centroided already) or *Never — show raw
profile* (peak-pick nowhere; be aware the MS map will then plot every raw sample,
which is slow).
